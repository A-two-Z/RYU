...
//    private static final String ROUTING_KEY = "robot.#";
	 ...
	 * 큐(Queue) 정의
	 * 
	 * @return Queue 객체. 이름은 'robot.queue'이며, durable(내구성) 속성은 false로 설정됨. durable이
	 *         false이면, RabbitMQ가 재시작되었을 때 큐가 삭제됩니다.
	 */
	@Bean
	Queue queue() {
	 ...
	 * 
	 * @return TopicExchange 객체. 이름은 'robot.exchange'. TopicExchange는 메시지를 라우팅 키의
	 *         패턴을 기반으로 여러 큐에 라우팅할 수 있습니다.
	 */
	@Bean
	 ...
	 * @return Binding 객체. 큐와 익스체인지를 'robot.#' 라우팅 키로 바인딩합니다. 'robot.#'는 'robot.'으로
	 *         시작하는 모든 라우팅 키와 일치합니다.
	 */
	@Bean
