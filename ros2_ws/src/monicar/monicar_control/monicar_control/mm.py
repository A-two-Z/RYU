...
class VehicleNode(Node):
    ...
    def cb_teleop(self, msg):
        ...
        speed_pulse = self.SPEED_CENTER + msg.linear.x * 12 * self.SPEED_LIMIT
        speed_pulse = clamp(speed_pulse, -self.SPEED_LIMIT + 1, self.SPEED_LIMIT - 1)

        
        if abs(msg.angular.z) >= 0.05:
            self.forward = True
            while abs(msg.angular.z) >= 0.05:
                if msg.angular.z > 0:
                    # Right turn
                    self._steering.run(410)
                else:
                    # Left turn
                    self._steering.run(290)        
    ...
    # Recalculate speed and steering pulse
            speed_pulse = self.SPEED_CENTER + msg.linear.x * 12 * self.SPEED_LIMIT
            
            speed_pulse = clamp(speed_pulse, -self.SPEED_LIMIT + 1, self.SPEED_LIMIT - 1)
            
            
            if steering_pulse > 350 and steering_pulse <= 353:
                steering_pulse = 380
            elif steering_pulse > 353:
                steering_pulse = 410
            elif steering_pulse < 350 and steering_pulse >= 347:
                steering_pulse = 320
            elif steering_pulse < 347:
                steering_pulse = 290
            else:
                steering_pulse = 350
